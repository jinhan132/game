...
<div id="tabmenu">
	<ul class="betmenu"> 
		...
		<li id="bettabmenuresult" class="bmenu_on" style="width:33%">완료내역</li>
...
?>
	<div class="page">
<?php	if($backPage >= 1){ ?>
		<a href="javascript:pageLoadPage('<?=$backPage?>');"><span class="box1">이 전</span></a>
<?php	}else{ ?>
...
<?php
		...
		else
		{
			...
			{
?>
			<a href="javascript:pageLoadPage('<?=$i?>');"><?=$i?></a>
<?php
			}
		}
...
<?php	if($nextPage <= $totalPage){ ?>
		<a href="javascript:pageLoadPage('<?=$nextPage?>');"><span class="box1">다 음</span></a>
<?php	}else{ ?>
